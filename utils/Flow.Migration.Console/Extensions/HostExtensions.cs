...
public static class HostExtensions
{
    public static async Task MigrateDatabaseAsync<TContext>(this IHost host, int retry = 0, int retryAmount = 3, CancellationToken cancellationToken = default) where TContext : DbContext
    {
        ...
        try
        {
            ...
            logger.LogInformation("End migrating database associated with context {DbContextName}.", typeof(TContext).Name);
        }
        catch (Exception exc)
        {
            logger.LogError(exc, "An error occurred while migrating the database used on context {DbContextName}.", typeof(TContext).Name);
